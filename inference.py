...
def test(args, model, test_loader, device):
    ...
    for test_batch in test_loader:
        ...
        with torch.no_grad():
            if args.model == 'bert' or 'bert' in args.model or 'minilm' in args.model.lower() or 'electra' in args.model.lower():
...
def main():
    ...
    if args.model == 'bert' or 'bert' in args.model:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.vocab)
        print('reading test data...')
        test_set = om.data.datasets.MiniLMDataset(
            dataset=args.test,
            tokenizer=tokenizer,
            mode='test',
            query_max_len=args.max_query_len,
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.vocab)
        print('reading test data...')
        test_set = om.data.datasets.ElectraDataset(
            dataset=args.test,
            tokenizer=tokenizer,
            mode='test',
            query_max_len=args.max_query_len,
