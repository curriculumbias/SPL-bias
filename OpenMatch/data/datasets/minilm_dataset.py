...
class MiniLMDataset(Dataset):
    ...
    def pack_minilm_features(self, query_tokens: List[str], doc_tokens: List[str]):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                input_ids_pos, input_mask_pos, segment_ids_pos = self.pack_minilm_features(query_tokens, doc_tokens_pos)
                input_ids_neg, input_mask_neg, segment_ids_neg = self.pack_minilm_features(query_tokens, doc_tokens_neg)
                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos, 'input_mask_pos': input_mask_pos,
                        'input_ids_neg': input_ids_neg, 'segment_ids_neg': segment_ids_neg, 'input_mask_neg': input_mask_neg}
            elif self._task == 'classification':
                query_tokens = self._tokenizer.tokenize(example['query'])[:self._query_max_len]
                ...
                input_ids, input_mask, segment_ids = self.pack_minilm_features(query_tokens, doc_tokens)
