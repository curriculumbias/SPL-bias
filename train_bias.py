...
def dev(args, model, metric, dev_loader, device):
    ...
    for dev_batch in dev_loader:
        ...
        with torch.no_grad():
            if args.model == 'bert'  or 'bert' in args.model or 'electra' in args.model.lower() or 'minilm' in args.model.lower():
...
def train_reinfoselect(args, model, policy, loss_fn, m_optim, m_scheduler, p_optim, metric, train_loader, dev_loader, device):
    ...
    for epoch in range(args.epoch):
        ...
        for step, train_batch in enumerate(train_loader):
            if args.model == 'bert'  or 'bert' in args.model:
        ...
        for step, train_batch in enumerate(tqdm(train_loader)):
            if args.model == 'bert'  or 'bert' in args.model:
                ...
                if args.task == 'ranking':
                    ...
                    raise ValueError('Task must be `ranking` or `classification`.')
            elif 'electra' in args.model.lower():
                if args.task == 'ranking':
                    batch_score_pos, _ = model(train_batch['input_ids_pos'].to(device), train_batch['input_mask_pos'].to(device), train_batch['segment_ids_pos'].to(device))
                    batch_score_neg, _ = model(train_batch['input_ids_neg'].to(device), train_batch['input_mask_neg'].to(device), train_batch['segment_ids_neg'].to(device))
                elif args.task == 'classification':
...
def main():
    ...
    parser.add_argument('-num_buckets', type=int, default=1000)
    ...
    if args.model == 'bert'  or 'bert' in args.model:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.vocab)
        print('reading training data...')
        train_set = om.data.datasets.MiniLMDataset(
            dataset=args.train,
            tokenizer=tokenizer,
            mode='train',
            query_max_len=args.max_query_len,
        ...
        print('reading dev data...')
        dev_set = om.data.datasets.MiniLMDataset(
            dataset=args.dev,
            tokenizer=tokenizer,
            mode='dev',
            query_max_len=args.max_query_len,
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.vocab)
        print('reading training data...')
        train_set = om.data.datasets.ElectraDataset(
            dataset=args.train,
            tokenizer=tokenizer,
            mode='train',
            query_max_len=args.max_query_len,
        ...
        print('reading dev data...')
        dev_set = om.data.datasets.ElectraDataset(
            dataset=args.dev,
            tokenizer=tokenizer,
            mode='dev',
            query_max_len=args.max_query_len,
    ...
    sampler = WeightedRandomSampler(train_set._create_weights(), num_samples=len(train_set), replacement=False)
    ...
    if args.model == 'bert'  or 'bert' in args.model or args.model == 'roberta':
        if args.maxp:
        ...
        model = om.models.MiniLM(
            pretrained=args.pretrain,
            mode=args.mode,
            task=args.task
        )
    elif 'electra' in args.model.lower() :
        model = om.models.Electra(
            pretrained=args.pretrain,
            mode=args.mode,
            task=args.task
        )
